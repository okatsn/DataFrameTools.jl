...
ignoring = "ignoring `missing`, `NaN` and `nothing`"

"""
`ifstrparse(x, TYPE)`: if `x` is `AbstractString`, then parse it to the variable of `TYPE`, $ignoring (ignoring those that `isnnm(x)` returns `true`).
"""
function ifstrparse(x, TYPE)
    if !isnnm(x) && isa(x, AbstractString)
        return parse(TYPE, x)
    else
        return x
    end
